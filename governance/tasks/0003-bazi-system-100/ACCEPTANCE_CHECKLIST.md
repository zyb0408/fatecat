...
# Global Standards
- [x] 任何 100% 结论都有可执行命令或可检查文件
- [x] 任一高级规则都有 sourceRuleId、conditions、doesNotApplyWhen、riskBoundary
- [x] 任一专题断法都有 benchmark/golden 或明确 evidence_seed 边界
- [x] MingLi 评估按 total/answered/correct/accuracy/byCategory 输出
- [x] full golden 不进入 quick；deep/release 使用 shard 或预算说明
- [x] 无 license 参考源不作为 production dependency
- [x] 新增代码/文档同步 AGENTS.md 或任务上下文

# Task Package Checklists
## TP-00
- 标题: 版本与基线控制面
- 验收项:
  - [x] `版本与基线控制面` 达到其 objective，且依赖关系保持一致
- Verify: 核对目标完成并补充执行证据
- Gate: 任务目标与上下文已确认
- 输出物:
  - [x] 在干净版本边界上建立八字 100% 的 scorecard 和当前缺口基线
- 标准清单:
...
- 验收项:
  - [x] commit hash 可追溯
  - [x] 后续 0003 计划改动不混入 checkpoint
- Verify: git log --oneline -1 && git status --short
- Gate: 最新提交为质量 hardening checkpoint，且提交后可明确区分 0003 新改动
- 输出物:
  - [x] git commit checkpoint
- 标准清单:
  - [x] Verify: git log --oneline -1 && git status --short
  - [x] Gate: 最新提交为质量 hardening checkpoint，且提交后可明确区分 0003 新改动
  - [x] 完成后更新 `STATUS.md` 的 `Recent Evidence`
  - [x] 交付前完成 REVIEW / SHIP 自检
...
- 标题: 最终审查和交付
- 验收项:
  - [x] `最终审查和交付` 达到其 objective，且依赖关系保持一致
- Verify: 核对目标完成并补充执行证据
- Gate: 任务目标与上下文已确认
- 输出物:
  - [x] 汇总八字 100% 推进结果，区分 Done、WARN、Blocked，并生成最终交付证据
- 标准清单:
  - [x] Verify: 核对目标完成并补充执行证据
  - [x] Gate: 任务目标与上下文已确认
  - [x] 完成后更新 `STATUS.md` 的 `Recent Evidence`
...
- 标题: Closeout 和版本交付
- 验收项:
  - [x] `Closeout 和版本交付` 达到其 objective，且依赖关系保持一致
...
- 输出物:
  - [x] governance/tasks/0003-bazi-system-100/CLOSEOUT.md
  - [x] git delivery evidence
- 标准清单:
  - [x] Verify: python3 /home/lenovo/.codex/skills/auto-tasks/scripts/validate_task_docs.py --task-dir governance/tasks/0003-bazi-system-100 --phase closeout && git status --short
  - [x] Gate: closeout 可校验；提交边界清晰；最终结论不越过证据
  - [x] 完成后更新 `STATUS.md` 的 `Recent Evidence`
  - [x] 交付前完成 REVIEW / SHIP 自检
