...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --sample)
      ...
      ;;
    --predictions-file)
      [[ $# -ge 2 ]] || usage_error "--predictions-file 缺少参数"
      predictions_file="$2"
      mode="evaluate"
      shift 2
      ;;
    --prompt-out)
      [[ $# -ge 2 ]] || usage_error "--prompt-out 缺少参数"
      prompt_out="$2"
      mode="prompt"
      shift 2
      ;;
    --output-json)
      [[ $# -ge 2 ]] || usage_error "--output-json 缺少参数"
      output_json="$2"
      shift 2
      ;;
    -h|--help)
...
def load_predictions(path: Path) -> dict[str, dict]:
    ...
    if path.suffix == ".jsonl":
        ...
        if isinstance(data, dict) and isinstance(data.get("predictions"), list):
            ...
            rows = [
                {"question_id": key, **value} if isinstance(value, dict) else {"question_id": key, "answer": value}
